...
(define fermat-test
  '(
    ...
    (while (> k 0)
            (
             ( a := (+ 2 ( rand (- n 4) ) ));;Losowa liczba od 2 do n-2
             (if(= (mod (expt a (- n 1))n) 1);;a^(n-1) mod n 
              (composite := false);;Raczej pierwsza w tym teście
              (
               (composite := true);;Złożona 
               (k := 0);;Kończy test
               ))   
              (k := (- k 1))
              ))                
...
;;TESTY
(probably-prime? 3 3);;3 Prime
(probably-prime? 8 3 );; 8 Composite
(probably-prime? 5 3 );;5 Prime
(probably-prime? 6 3 );;6 Composite
(probably-prime? 121 3 );;121 Composite
(probably-prime? 991 3 );;991 Prime
(probably-prime? 7919 4);;7919 Prime
(probably-prime? 7910 4);;7910 Composite