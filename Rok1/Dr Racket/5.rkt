...
(define (clause-trivial? c)
  (if(null?(cos(res-clause-pos c)
               (res-clause-neg c)))
     #f
     #t ))

(define (resolve c1 c2)
  (let((a(cos(res-clause-pos c1)
             (res-clause-neg c2)))
       (b(cos(res-clause-pos c2)
             (res-clause-neg c1))))
    (cond
      [(and(null? a)
          (null? b)) #f]
      [(not(null? a))
       (res-clause(remove-duplicates-vars
                   (merge-vars
                    (remove a(res-clause-pos c1))
                    (res-clause-pos c2)))
                  (remove-duplicates-vars
                   (merge-vars
                    (res-clause-neg c1)
                    (remove a(res-clause-neg c2))))
                  (proof-res a (res-clause-proof c1)(res-clause-proof c2)))]
      [else
       (res-clause(remove-duplicates-vars
                   (merge-vars(res-clause-pos c1)
                              (remove b (res-clause-pos c2))))
                  (remove-duplicates-vars
                   (merge-vars (remove b(res-clause-neg c1))
                               (res-clause-neg c2)))
                  (proof-res b(res-clause-proof c2)(res-clause-proof c1)))])))
...
;Tworze 3 Klauzule
(define a(res-clause '(a b) '(a b) (proof-res  'res-int '(a b) '(a b))))
(define b(res-clause '(s q) '(p  r) (proof-res  'res-int '(s q) '(p  r))))
(define c(res-clause '(r p) '(s q) (proof-res 'res-int '(r p) '(s q))))
;;Test czy działa funkcja clause-trivial?
(clause-trivial? a)
(clause-trivial? b)
(clause-trivial? c)
;;Test czy działa funkcja resolve
(resolve b c)
