...
(define (heap-merge h1 h2)
  (cond
   [(leaf? h1) h2]
   [(leaf? h2) h1]
   [else(if(>= (elem-priority (node-elem h1))
              (elem-priority(node-elem h2)))
           (make-node (node-elem h2)
                      (node-left h2)
                      (heap-merge h1 (node-right h2)))
           (make-node (node-elem h1)
                      (node-left h1)
                      (heap-merge h2 (node-right h1))))
...
(define (randlist len max)
  ...
  (aux len null))
;Pierwsza Lista
(define x (randlist 5 12))
;;Druga Lista
(define y(randlist 5 20))
x
;;Sortowanie listy
(heapsort x)
y
(heapsort y)
;;Predykat czy się udało posortować
(sorted? (heapsort x))
(sorted?(heapsort y))
;;Trzecia Lista
(define z(randlist 22 21))
;;Bez sortowania sprawdzam , jest w odpowiedniej kolejnosci
(sorted? z)